...
mod LizInventory {
    ...
    impl LizInventoryImpl of super::ILizInventory<ContractState> {
        fn add_stock(ref self: ContractState, product: felt252, new_stock: u32) {
            ...
            assert (caller == self.contract_owner.read(), 'Caller Owner Mismatch');
            let mut cur_stock = self.inventory.read(product);
            cur_stock += new_stock;
            self.inventory.write(product, cur_stock);
        }
        ...
        fn purchase(ref self: ContractState, product: felt252, quantity: u32) {
            ...
            let mut cur_stock = self.inventory.read(product);
            assert (cur_stock >= quantity, 'stock less than quantity');
            cur_stock -= quantity;
            self.inventory.write(product, cur_stock);
        }
